...
(function ($) {
    Sopa = function ($el, options) {
        ...
        var defaults = { palabras: [{ name: 'CONJUNTO' }, { name: 'CARDINALIDAD' }, { name: 'INTERSECCION' }, { name: 'UNION' }, { name: 'PRODUCTO'}, { name: 'RELACION'}, { name: 'MATRIZ'}, { name: 'REFLEXIVA' }, { name: 'SIMETRICA' }, { name: 'EQUIVALENCIA' }], complejo: 15, vertical: 'S', onWin: "" };
        var aciertos = 0;
        ...
        this.init = function () {
            ...
            for (var j = 0; j < defaults.complejo + 1; j++) {
                ...
                for (var i = 0; i < defaults.complejo + 1; i++) {
                    var pos = Math.floor((Math.random() * (defaults.complejo + 10)) - 1);
